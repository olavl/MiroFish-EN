...
# China daily schedule time config (Beijing time)
...
class SimulationConfigGenerator:
    ...
    to automatically generate optimal simulation parameter configs
...
## Task
Generate time config JSON.

### Basic Principles (for reference only, adjust flexibly based on specific events and participant groups):
- Target user group is Chinese, must follow Beijing time daily schedule
- 0-5am almost no activity (activity coefficient 0.05)
- 6-8am gradually active (activity coefficient 0.4)
- 9am-6pm work hours moderate activity (activity coefficient 0.7)
- 7-10pm evening peak (activity coefficient 1.5)
...
- **Important**: The example values below are for reference only, adjust based on event nature and participant characteristics
  - Example: Student peak may be 9-11pm; media active all day; official institutions only during work hours
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type alias map (handle different formats LLM may output)
        type_aliases = {
        ...
        # Track used agent index per type to avoid reusing same agent
...
- **Time follows Chinese daily schedule**: Almost no activity 0-5am, most active 7-10pm
- **Official institutions** (University/GovernmentAgency): Low activity(0.1-0.3), active during work hours(9-17), slow response(60-240min), high influence(2.5-3.0)
- **Media** (MediaOutlet): Medium activity(0.4-0.6), active all day(8-23), fast response(5-30min), high influence(2.0-2.5)
- **Individuals** (Student/Person/Alumni): High activity(0.6-0.9), mainly active in evening(18-23), fast response(1-15min), low influence(0.8-1.2)
