...
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze given text content and simulation requirements to design entity types and relationship types suitable for **social media opinion simulation**.
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person doesn't belong to other more specific person types, they fall into this category.
   - `Organization`: Fallback type for any organization. When an organization doesn't belong to other more specific organization types, it falls into this category.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for the main roles appearing in the text
   - Example: For academic events, you could have `Student`, `Professor`, `University`
   - Example: For business events, you could have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people appear in the text, such as "elementary school teachers", "random passerby", "some netizen"
