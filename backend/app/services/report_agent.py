...
4. Supports user conversations with autonomous retrieval tool calls
"""
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only, does not mean entire section is complete)"""
        self.log(
...
[Returns]
- Related facts in original text (can be directly quoted)
...
4. Calls /api/simulation/interview/batch endpoint to conduct real interviews on both platforms
...
and Agent behaviors and interactions are predictions of future human behavior.
...
3. [Language consistency - quoted content must be translated to the report language]
   ...
   - Keep the original meaning when translating, ensure natural and smooth expression
...
[Correct example]
```
This section analyzes the public opinion propagation trends of the event. Through deep analysis of simulation data, we found...
...
### 1. Initial Phase          <- Wrong! Don't use ### for sub-sections
#### 1.1 Detailed Analysis    <- Wrong! Don't use #### for sub-divisions
...
4. [Quote format requirement - must be standalone paragraphs]
   ...
   Wrong format:
   ```
   The university's response was considered lacking in substance. > "The university's response pattern..." This evaluation reflects...
   ```
...
class ReportAgent:
    ...
    def __init__(
        ...
        self.llm = llm_client or LLMClient(model="claude-opus")
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempt when format 1 didn't match, avoid false matching JSON in body text
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflicts of tool call and Final Answer appearing together
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls:
                # Tool quota exhausted -> clearly notify, require Final Answer output
