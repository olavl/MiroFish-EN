  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatibility: single platform marker)
      ...
      // Extract key quotes (supporting multiple quote formats)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.relations.length} relations`)
          ]),
          ...
          }, expandedRelations.value ? `Collapse ▲` : `Show All ${props.result.relations.length} Relations ▼`)
        ]),
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (supporting multiple formats)
  ...
  // If starting with ## or # or >, might be direct markdown content
