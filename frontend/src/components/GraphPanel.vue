...
// Watch isSimulating changes, detect simulation completion
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to non-simulating state, show completion hint
...
const renderGraph = () => {
  ...
  // Process edge data, compute edge counts and indices between node pairs
  const edgePairCount = {}
  ...
  const getLinkPath = (d) => {
    ...
    // Compute curve control points - dynamically adjust based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset based on distance ratio, ensuring visible curvature
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if real dragging started (moved beyond threshold)
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop if real dragging occurred
        if (d._isDragging) {
