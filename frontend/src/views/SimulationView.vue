...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`Failed to close sim env: ${closeRes.error || 'Unknown error'}`)
          // If graceful shutdown fails, try force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Sim env close error: ${closeErr.message}`)
    ...
    // Failed environment status check does not affect subsequent flow
